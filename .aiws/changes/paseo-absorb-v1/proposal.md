...
- 需求交付：`Req_ID` = REQUIREMENTS.md `H3`（tool/permission）、`M2`（web 实时）、`M3`（审批）、`M5`（sessions/messages）+ spool replay 验收
- 问题修复：无（`issues/` 目录不存在，未建问题工件）
- 合同行：`Contract_Row` = TBD（`requirements/requirements-issues.csv` 不存在，待补最小合同或 `ws-req-change`）
- 计划文件：`plan/2026-09-17_11-04-38-paseo-absorb.md`（plan-verify 已通过：`ok: plan verification passed`）
...
- 风险：
  ...
  - `Contract_Row` 为 TBD，若门禁要求合同行需先补最小合同。
- 回滚方案（必须可执行）：
...
- `requirements/requirements-issues.csv`：TBD——目录不存在，`Contract_Row` 暂空；门禁若拦则补最小合同
