...
- [ ] 0.3 若真值文件发生变化（例如你更新了 REQUIREMENTS.md），同步基线：`aiws change sync web-unit-blocks`
...
- [ ] 2.1 新增 reduce.test.ts（配对/ok判定/过滤合并/pinned跳过）
- [ ] 2.2 新增 reconcile.test.ts（引用保持/变更替换/顺序变化）

## 3. 验证（必须可复现）

- [ ] 3.1 bash -c "cd web && bun run test:unit" 全绿
- [ ] 3.2 bash -c "cd web && bun run build" exit 0

## 4. 交付与归档

- [ ] 4.1 证据落盘到 `.agentdocs/tmp/...`（报告/日志/请求响应等）
- [ ] 4.2 交叉审计（可选但推荐）：在 AI 工具内运行 `/ws-review`（或按 `AI_PROJECT.md` 手工审计）
- [ ] 4.3 归档：`aiws change archive web-unit-blocks`
